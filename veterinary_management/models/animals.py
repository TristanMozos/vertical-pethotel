...
class Animal(models.Model):
    ...
    sex = fields.Selection(
        [("male", "Male"), ("female", "Female")], string="Sex", default="male"
    )
    ...
    medicines = fields.Many2many(
        "animal.medicine", string="Medicines", relation="animal_medicine_rel"
    )
    vaccines = fields.Many2many(
        "animal.vaccine", string="Vaccines", relation="animal_vaccine_rel"
    )
    diseases = fields.Many2many(
        "animal.disease", string="Diseases", relation="animal_disease_rel"
    )
    allergies = fields.Many2many(
        "animal.allergy", string="Allergies", relation="animal_allergy_rel"
    )
    surgeries = fields.Many2many(
        "animal.surgery", string="Surgeries", relation="animal_surgery_rel"
    )
    ...
    insurance = fields.Many2one(
        "animal.insurance", string="Insurance", relation="animal_insurance_rel"
    )
    ...
    def create(self, vals):
        if vals.get("identification", "New") == "New":
            vals["identification"] = (
                self.env["ir.sequence"].next_by_code("animal.identification") or "New"
            )
    ...
    def _compute_quote_count(self):
        for record in self:
            if record.owner:
                record.quote_count = self.env["sale.order"].search_count(
    ...
    def _compute_visit_count(self):
        for record in self:
            if record.id:
                record.visit_count = self.env["animal.visit"].search_count(
                    [("animal_id", "=", record.id)]
                )
            else:
